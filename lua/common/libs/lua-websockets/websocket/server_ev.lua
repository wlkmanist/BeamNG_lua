...
local frame = require'libs/lua-websockets/websocket.frame'
local handshake = require'libs/lua-websockets/websocket.handshake'
...
local client = function(sock,protocol)
  ...
  local message_io
  local close_timer
  local async_send = require'libs/lua-websockets/websocket.ev_common'.async_send(sock,loop)
  local self = {}
  self.state = 'OPEN'
  self.sock = sock
  local user_on_error
  ...
  self.start = function()
    message_io = require'libs/lua-websockets/websocket.ev_common'.message_io(
      sock,loop,
      on_message,
    handle_sock_err)
  end
