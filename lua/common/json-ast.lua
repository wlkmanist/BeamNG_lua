...
-- this creates a tiny lookup table that contains the first node idx for each line. Used in the text editor
local function _createLinesBuffer(ctx)
  local lines = {}
  local firstNodeIdx = nil
  for nodeIdx, node in ipairs(ctx.ast.nodes) do
    local nodeType = node[1]
    if not firstNodeIdx then
  ...
  -- Last line doesn't have newline characters
  if firstNodeIdx then
    table.insert(lines, firstNodeIdx)
  end

  ctx.transient.linesIndexes = lines
end

-- creates a lookup table to get the line number from node index
local function _createNodeIdxToLineBuffer(ctx)
  local tbl = {}
  local lineNum = 1
  for nodeIdx, node in ipairs(ctx.ast.nodes) do
    local nodeType = node[1]
  ...
  ctx.transient.nodeIdxToLineNum = tbl
end
...
local function parse(str, addAstId)
  ...
  _createLinesBuffer(ctx)
  _createNodeIdxToLineBuffer(ctx)
