...
--[[ Usage
  ...
  -- Query:
  Two ways to query the items in the tree.

  1) queries that are not nested
  for item_id in q:queryNotNested(query_xmin, query_ymin, query_xmax, query_ymax) do
    -- do something with item_id --
  end

  2) nested queries (also works for non nested queries but will be slower and create garbage)
  for item1 in q:query(query_xmin, query_ymin, query_xmax, query_ymax) do -- for all the items in the tree that intersect the querry area
    for item2 in q:query(item1_xmin, item1_ymin, item1_xmax, item1_ymax) do
      -- do stuff --
    end
  end
--]]
