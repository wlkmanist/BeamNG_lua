...
local function prof_annotate(count1, samples)
  ...
  for _, file in ipairs(files) do
    ...
    if ann ~= 0 then
      for i=1,ann do
  if fl[i] then show = true; outString = outString .. ("@@ 1 @@\n"); break end
      end
    end
...
local function prof_finish()
  if prof_ud then
    ...
    if samples == 0 then
      if prof_raw ~= true then outString = outString .. ("[No samples collected]\n") end
      return
    end
    if prof_ann then
...
function M.onUpdate(dtReal, dtSim, dtRaw)
  ...
  prof_finish()
  if outString ~= '' then
    outStringCache = outString
    outString = ''
  end
  prof_start("vzm0lspi0 500 -500")
