...
local function new(itemCount)
  return setmetatable({
    tree = nil,
    nonLeafLimIdx = nil,
    items = table.new(max(0, itemCount or 0)*4, 0),
    itemCount = 0,
...
local function query_it(st)
  ...
  if nodeIdx > nonLeafLimIdx then
    for i = max(tree[nodeIdx-2], st.itmIdx), tree[nodeIdx], 4 do
      if items[i-3] >= queryArea[1] and items[i-3] <= queryArea[4] and items[i-2] >= queryArea[2] and items[i-2] <= queryArea[5] and items[i-1] >= queryArea[3] and items[i-1] <= queryArea[6] then
        st.itmIdx = i + 4 -- update
        return items[i]
      end
    end
    st.itmIdx = -1 -- reset
    nodeIdx, axis = stack[st.stackIdx-1], stack[st.stackIdx] -- pop stack
  ...
  repeat
    ...
    if tree[nodeIdx-3] <= queryMax and tree[nodeIdx-2] >= queryMin then -- left subtree
      axis = axis4Next[axis]
      if tree[nodeIdx-1] <= queryMax and tree[nodeIdx] >= queryMin then -- right subtree
        st.stackIdx = st.stackIdx + 2
        stack[st.stackIdx-1], stack[st.stackIdx] = 2 * nodeIdx + 4, axis -- push right subtree to stack
      end
      nodeIdx = 2 * nodeIdx
    elseif tree[nodeIdx-1] <= queryMax and tree[nodeIdx] >= queryMin then -- right subtree
      axis = axis4Next[axis]
      nodeIdx = 2 * nodeIdx + 4
    else
      if nodeIdx > nonLeafLimIdx then
...
function kdTree:query(query_xmin, query_ymin, query_zmin, query_xmax, query_ymax, query_zmax)
  return query_it, {
    ...
    items = self.items,
    nonLeafLimIdx = self.nonLeafLimIdx,
    queryArea = {max(query_xmin, -1.7e308), max(query_ymin, -1.7e308), max(query_zmin, -1.7e308), min(query_xmax, 1.7e308), min(query_ymax, 1.7e308), min(query_zmax, 1.7e308)},
    stack = {},
    stackIdx = 0,
    curNodeIdx = 4,
    curAxis = 1,
    itmIdx = -1 -- keeps track of item idx when traversing the items in a leaf node
  }
end

function kdTree:queryNotNested(query_xmin, query_ymin, query_zmin, query_xmax, query_ymax, query_zmax)
  self.queryArea[1] = max(query_xmin, -1.7e308)
  self.queryArea[2] = max(query_ymin, -1.7e308)
  self.queryArea[3] = max(query_zmin, -1.7e308)
  self.queryArea[4] = min(query_xmax, 1.7e308)
  self.queryArea[5] = min(query_ymax, 1.7e308)
  self.queryArea[6] = min(query_zmax, 1.7e308)
  self.curNodeIdx = 4
  self.curAxis = 1
  self.itmIdx = -1
  self.stackIdx = 0
