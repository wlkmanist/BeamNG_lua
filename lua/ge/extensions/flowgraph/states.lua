...
function C:startState(id, transData)
  ...
  if self.mgr.activity and self.mgr.activity.onFlowgraphStateStarted then
    self.mgr.activity:onFlowgraphStateStarted(state.name, state, transData)
  end
  state.active = true
...
function C:stopState(id)
  ...
  if self.mgr.activity and self.mgr.activity.onFlowgraphStateStopped then
    self.mgr.activity:onFlowgraphStateStopped(state.name, state)
  end
  state.extProxy:submitEventSinks({},{})
  state.active = false
