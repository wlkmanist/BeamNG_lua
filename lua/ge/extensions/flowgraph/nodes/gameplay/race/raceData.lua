...
C.pinSchema = {
  ...
  {dir = 'out', type = 'bool', name = 'isOnRoad', description = 'True while the vehicle is within the bounds of the race AI path.'}
}
...
function C:work(args)
  ...
  local isOnRoad = state.isOnRoad
  if not isOnRoad then
    if not self._frameDelay then -- isOnRoad might be false for only one tick
      self._frameDelay = 1
      isOnRoad = true
    end
  else
    self._frameDelay = nil
  end
  self.pinOut.isOnRoad.value = isOnRoad
end
