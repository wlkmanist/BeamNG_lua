...
C.name = 'Activate Vehicle'
C.description = 'Activates or deactivates a vehicle in the pool.'
...
C.pinSchema = {
  ...
  { dir = 'in', type = 'bool', name = 'activate', default = true, description = 'Activate or deactivate this vehicle.' }
}
...
function C:workOnce()
  if self.pinIn.vehPool.value and self.pinIn.vehId.value then
    self.pinIn.vehPool.value:setVeh(self.pinIn.vehId.value, self.pinIn.activate.value)
  end
end
