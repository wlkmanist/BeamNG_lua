...
-- Global debug settings (simplified, no per-rope persistence)
local debugDraw = true
local debugNodes = true
local debugDir = true
local debugText = false
local debugMass = false
...
-- Get ropes for the current vehicle only
local function getCurrentVehicleRopes()
  local vehicle = be:getPlayerVehicle(0)
  if not vehicle or not vehicle.ropeIds then
    return {}
  end
...
local function updateUIPtrs()
  if selectedRope then
    -- Set base positions from rope's basePosA/basePosB properties for editing
    uiPtrs.anchorAX[0] = selectedRope.basePosA and selectedRope.basePosA.x or 0
    uiPtrs.anchorAY[0] = selectedRope.basePosA and selectedRope.basePosA.y or 0
    uiPtrs.anchorAZ[0] = selectedRope.basePosA and selectedRope.basePosA.z or 0
    uiPtrs.anchorBX[0] = selectedRope.basePosB and selectedRope.basePosB.x or 0
    uiPtrs.anchorBY[0] = selectedRope.basePosB and selectedRope.basePosB.y or 0
    uiPtrs.anchorBZ[0] = selectedRope.basePosB and selectedRope.basePosB.z or 0
    uiPtrs.baseDirAX[0] = selectedRope.baseDirA and selectedRope.baseDirA.x or 0
    uiPtrs.baseDirAY[0] = selectedRope.baseDirA and selectedRope.baseDirA.y or 0
    uiPtrs.baseDirAZ[0] = selectedRope.baseDirA and selectedRope.baseDirA.z or -1
    uiPtrs.baseDirBX[0] = selectedRope.baseDirB and selectedRope.baseDirB.x or 0
    uiPtrs.baseDirBY[0] = selectedRope.baseDirB and selectedRope.baseDirB.y or 0
    uiPtrs.baseDirBZ[0] = selectedRope.baseDirB and selectedRope.baseDirB.z or -1
    ...
    -- Use global debug settings
    uiPtrs.debugDraw[0] = debugDraw
    uiPtrs.debugNodes[0] = debugNodes
    uiPtrs.debugDir[0] = debugDir
    uiPtrs.debugText[0] = debugText
    uiPtrs.debugMass[0] = debugMass
...
local function drawRopeNodes(rope)
  ...
  if debugNodes then
    ...
    for i = 1, #nodes do
      ...
      if i == 1 then
        -- Anchor A - always blue for legend consistency
        nodeCol = ColorF(0.2, 0.4, 0.8, 1.0)  -- Blue for Anchor A
  ...
  if debugText then
    ...
    -- Anchor status text
    local anchorAText = rope.anchorAFixed and "A: FIXED" or "A: FREE"
    local anchorBText = rope.anchorBFixed and "B: FIXED" or "B: FREE"
...
local function createRopeFromUI()
  ...
  if id then
    ...
    if rope then
      ...
      -- Cache this rope and set base positions
      selectedRope = rope
...
local function onUpdate(dt)
  ...
  if im.Begin("Rope Visual Sandbox", windowOpen) then
    ...
    -- Get existing ropes (no creation/management UI)
    local allRopes = getCurrentVehicleRopes()

    -- Auto-select first rope if available
    ...
    if selectedRope then
      ...
      if im.BeginTable("##AnchorTable", 7, im.TableFlags_BordersInnerV) then
        ...
        -- Header row with anchor indicators
        im.TableNextRow()
