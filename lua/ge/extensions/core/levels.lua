...
local function _findAvailableLevels()
  ...
  for _, d in pairs(level_dirs) do
    if shouldHideUNRELIABLE("/levels/", d) then
      goto continue
    end
      ...
      if not career_career.isActive() or career_modules_spawnPoints.isSpawnPointDiscovered(levelInfo.levelName, spawnPoint.objectname) then
        ...
        if obj then
          ...
          table.insert(elements,  {
            ...
            data = data,
            markerInfo = {
              bigmapMarker = {pos = obj:getPosition(), icon = "poi_fasttravel_round_orange_green",  quickTravelPosRotFunction = getSpawnPointPosRot, name = data.translationId, description = data.description, thumbnail = data.previews[1], previews = data.previews}
            }
          })
        else
