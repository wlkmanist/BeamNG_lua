...
function C:init()
  self.fovMin = 50 -- This will be overwritten in bigMapMode.lua based on map size
  self.fovMax = 1000
  self.angle = 53
  self.rotAngle = 25
...
function C:setCustomData(data)
  ...
  core_orthoCamera.enable(true, {size = avgSideLength})
  self.firstUpdate = true
  self.fovMax = avgSideLength
  self.manualzoom:init(self.fovMax, self.fovMin, self.fovMax)
end
...
local orthoOptions = {size = 1, nearClip = 1, farClip = 5000}
function C:update(data)
  ...
  orthoOptions.size = data.res.fov
  local zoomRelative = (data.res.fov - self.fovMin) / (self.fovMax - self.fovMin)
  orthoOptions.nearClip = self.bigMapCamPosition.z * lerp(0.5, 2*0.3, 1-zoomRelative)
  orthoOptions.farClip = self.bigMapCamPosition.z * 5

  core_orthoCamera.setOptions(orthoOptions)
  return true
end
