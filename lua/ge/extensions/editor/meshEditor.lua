    ...
    if M.type == "River" then
      if index < mesh:getNodeCount() then
        ...
        if camPos:distance(middlePos) < renderDistance then
          ...
          debugDrawer:drawLineInstance(p1, p2, lineWidth, roadRiverGui.highlightColors.selected, false)
          debugDrawer:drawLineInstance(p2, p3, lineWidth, roadRiverGui.highlightColors.selected, false)
          debugDrawer:drawLineInstance(p3, p1, lineWidth, roadRiverGui.highlightColors.selected, false)
        end
      end
    end
  end
...
local function showMesh(mesh, meshColor)
  ...
  for index = 0, mesh:getEdgeCount() - 1 do
    ...
    if index < mesh:getEdgeCount() - 1 then
      ...
      debugDrawer:drawLineInstance(topLeft1, mesh:getTopLeftEdgePosition(index+1), borderLineWidth, meshColor, false)
      debugDrawer:drawLineInstance(topMiddle1, mesh:getTopMiddleEdgePosition(index+1), midLineWidth, meshColor, false)
      debugDrawer:drawLineInstance(topRight1, mesh:getTopRightEdgePosition(index+1), borderLineWidth, meshColor, false)

      if debugDrawDetail >= 2 and M.type == "River" then
        if debugDrawDetail == 3 or (index % round(10 / segmentLength)) == 0 then
          debugDrawer:drawLineInstance(bottomLeft1, mesh:getBottomLeftEdgePosition(index+1), borderLineWidth, meshColor, false)
          debugDrawer:drawLineInstance(bottomRight1, mesh:getBottomRightEdgePosition(index+1), borderLineWidth, meshColor, false)
        end
      end
    end
    ...
    if debugDrawDetail >= 2 then
      if debugDrawDetail == 3 or (index % round(10 / segmentLength)) == 0 then
        local lineWidth = editor.getPreference("gizmos.general.lineThicknessScale") * 4
        debugDrawer:drawLineInstance(topLeft1, topRight1, lineWidth, meshColor, false)

        if M.type == "River" then
          debugDrawer:drawLineInstance(topLeft1, bottomLeft1, lineWidth, meshColor, false)
          debugDrawer:drawLineInstance(bottomLeft1, bottomRight1, lineWidth, meshColor, false)
          debugDrawer:drawLineInstance(bottomRight1, topRight1, lineWidth, meshColor, false)
        end
      end
    end
  end
  ...
  if rayCastNoMesh and rayCastNoMesh.pos then
    ...
    if im.IsMouseClicked(0) and not (im.IsAnyItemHovered() or im.IsWindowHovered(im.HoveredFlags_AnyWindow)) then
      ...
      -- Clicked while in create mode
        if selectedMesh then
          local nodeIdx = selectedMesh:collideRay(core_camera.getPosition(), (focusPointNoMesh - core_camera.getPosition()))
