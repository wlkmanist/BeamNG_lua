      ...
      if data.lanes then
        ...
        for i = 1, laneCount do -- draw lanes
          ...
          if false and mapNodes[nid].isJunction then -- project lanes from previous link towards intersection
            ...
            local prevNode
            for k, v in pairs(gp.graph[lid]) do
              if k ~= nid then
                prevNode = k
              end
            end
            ...
            if prevNode then
              local projDir = mapNodes[lid].pos - mapNodes[prevNode].pos
              local p1, p2 = pos1 - right1 * mapNodes[lid].radius, pos1 + right1 * mapNodes[lid].radius
              local xnorm1, xnorm2 = closestLinePoints(p1, p2, mapNodes[lid].pos, mapNodes[lid].pos + projDir * 10)
              pos1 = linePointFromXnorm(p1, p2, xnorm1)
              rad1 = rad2
            end
          end
    ...
    if not qtNodes or updateQt then
      ...
      if mapNodes then
        for nid, n in pairs(mapNodes) do
          ...
          local nDrivability = be:getTerrainDrivability(n.pos, n.radius)
          if nDrivability <= 0 then nDrivability = 1 end
          for lid, data in pairs(n.links) do
            local lPos = mapNodes[lid].pos
            local lRad = mapNodes[lid].radius
            local edgeDrivability = math.min(1, math.max(1e-30, (be:getTerrainDrivability(lPos, lRad) + nDrivability) * 0.5 * data.drivability))
            local inNode = data.inNode
            local outNode = inNode == nid and lid or nid
            if data.oneWay then
              gp:uniEdge(inNode, outNode, nPos:distance(lPos) / edgeDrivability, data.drivability, data.speedLimit, data.lanes, data.oneWay)
            else
              gp:bidiEdge(inNode, outNode, nPos:distance(lPos) / edgeDrivability, data.drivability, data.speedLimit, data.lanes, data.oneWay)
            end
          end
        end
      end
...
local function onEditorAfterSaveLevel()
  if navgraphDirty then
    ...
    navgraphDirty = false
    log('I', logTag, "Saved navgraph to file")
  end
end
