...
function C:getStarLabel(key)
  local l = self.missionInstance.starLabels[key]
  if type(l) == "function" then

    l = l(self.missionInstance)
  end
...
function C:drawDefaultStar(defaultIndex)
  ...
  if self.mission.careerSetup.defaultStarKeys[defaultIndex] then
    im.tooltip(_tr(self.missionInstance.starLabels[self.mission.careerSetup.defaultStarKeys[defaultIndex]]))
  end
end
...
function C:drawStarRewards(key)
  ...
  if remIdx then
    ...
    self.mission._dirty = true
    table.clear(inputBuffers)
  end
  local added = self:drawAddReward(key, rewards)
  if added then
    self.mission._dirty = true
    self.mission.careerSetup.starRewards[key] = added
  end
  im.PopID()
end
...
function C:drawOutroText(key)
  ...
  if not hasDefault then
    ...
    if usingDefault then
      im.tooltip("Using Default Translation: " ..self.missionInstance.defaultStarOutroTexts[key].." : " .._tr(self.missionInstance.defaultStarOutroTexts[key]))
    else
      im.tooltip("Available Default Translation: " ..self.missionInstance.defaultStarOutroTexts[key].." : " .._tr(self.missionInstance.defaultStarOutroTexts[key]))
    end
  end
  ...
  if not self._translatedTexts[key] then
    self._translatedTexts[key] = _tr(self.mission.careerSetup.starOutroTexts[key] or "", noTranslation)
  end
...
function C:drawCareerSetup()
  ...
  for i, key in ipairs(self.starKeysSorted) do
    ...
    if self.mission.careerSetup.starsActive[key] then
      im.Columns(2, "objectivesSlotColumns" .. tostring(key))
      ...
      im.Columns(2, "objectivesRewardColumns" .. tostring(key))
      im.SetColumnWidth(0,150)
      im.Separator()
      im.Text("Rewards")
      im.NextColumn()
