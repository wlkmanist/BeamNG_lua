...
local showMain = true
M.lastOpenedFolder = 'flowgraphEditor/'
local autoSaveDir = "flowgraphEditor/autoSaves/"
...
local function saveMacroAs(graph)
  ...
  if not tableIsEmpty(dirtyChildren) then
    ...
    extensions.editor_fileDialog.saveFile(function(data)mgr:saveMacro(graph, data)end, {{"Node graph Files",".macro.flow.json"}}, false, "/flowgraphEditor/macros/")
  end
end
