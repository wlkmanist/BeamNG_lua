...
local visualPacenotesFname = aipSettingsRoot..'/visualPacenotes.json'
local pacenotesTranslationExt = 'pacenotesTranslation.json'
...
local default_notebook_basename = default_notebook_name..'.'..notebookFileExt
...
local default_structured_pacenote_style = '1_to_6'
...
local function buildPacenotesTranslationFname(lang)
  return aipSettingsRoot..'/languages/'..lang..'.'..pacenotesTranslationExt
end

local function loadPacenotesTranslationFile(lang)
  local filename = buildPacenotesTranslationFname(lang)
  local json = jsonReadFile(filename)
  if json then
    return json, nil
  else
    error('unable to find pacenotesTranslation file: ' .. tostring(filename))
  end
end

local function loadVisualPacenotesFile()
  local filename = visualPacenotesFname
  local json = jsonReadFile(filename)
  if json then
    return json, nil
  else
    error('unable to find visualPacenotes file: ' .. tostring(filename))
  end
end

local function getStructuredPacenoteStyle()
  return default_structured_pacenote_style
end
...
M.default_notebook_basename = default_notebook_basename
...
M.loadPacenotesTranslationFile = loadPacenotesTranslationFile
M.loadVisualPacenotesFile = loadVisualPacenotesFile

M.getStructuredPacenoteStyle = getStructuredPacenoteStyle
