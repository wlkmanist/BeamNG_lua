...
function C:createMouseDragPacenote()
  ...
  if self.mouseInfo.down then
    ...
    if pos_down then
      ...
      normalVec = self.pacenote_tools_state.snaproad:forwardNormalVec(point_cs)
      if normalVec then
        wp_cs:setNormal(normalVec)
      end

      normalVec = self.pacenote_tools_state.snaproad:forwardNormalVec(point_ce)
      if normalVec then
...
function C:drawPacenotesList()
  ...
  for i, note in ipairs(notebook.pacenotes.sorted) do
    ...
    if self.path:useStructured() then
      tooltipText = dumps(note:joinedStructured(lang))
    else
      tooltipText = note:getNoteFieldNote(lang)
    end
...
- 'currLap < maxLap' -> audio will play except for on the last lap
    ...
    -- for i,lang_data in ipairs(self.path:getLanguages()) do
      ...
      StructuredForm.draw(pacenote, language)
