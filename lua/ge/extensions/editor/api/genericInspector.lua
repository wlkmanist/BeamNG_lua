    ...
    for _, val in ipairs(fields) do
      ...
      if not val.isArray and not val.hidden then
        if val.elementCount == nil or val.elementCount == 1 then
          self.valueInspector:valueEditorGui(val.name, val.value or "", 0, val.name, val.fieldDocs, val.type, val.typeName, val, function (fieldName, fieldValue, arrayIndex, customData) self.fieldSetter(fieldName, fieldValue, arrayIndex, customData, true) end, contextMenuUI, val.indeterminateFlags)
        else
            ...
            if imgui.CollapsingHeader1("[" .. tostring(i - 1) .. "]", nodeFlags) then
              for _, arrayField in ipairs(val.sortedFields) do
                if not arrayField.hidden then
                  ...
                  self.valueInspector:valueEditorGui(arrayFieldName, arrayField.value[i] or "", i, arrayFieldName, arrayField.fieldDocs, arrayField.type, arrayField.typeName, arrayField, function (fieldName, fieldValue, arrayIndex, customData) self.fieldSetter(fieldName, fieldValue, arrayIndex, customData, true) end, contextMenuUI, val.indeterminateFlags)
                end
              end
            end
            imgui.PopID()
