...
local allKnownContexts = {"headsUpRace", "bracketRace", "loopRace", "tuffTrucksRace", "dragPracticeRace", "streetRodRace", "streetDogfightRace"}
local allKnownPhases = {"stage", "prestage", "start", "race", "stop"}
...
local function drawElementDetail(elem)
  ...
  for _, key in ipairs({"spawn","stage", "endLine", "boundary"}) do
    im.Text(key.." waypoint:")
    if transforms[key] and transforms[key]:update(mouseInfo) then
      elem[key].transform.pos = transforms[key].allowTranslate and transforms[key].pos or nil
      elem[key].transform.rot = transforms[key].allowRotate and transforms[key].rot or nil
      elem[key].transform.scl = transforms[key].allowScale and transforms[key].scl or nil
    end
    if elem[key] ~= nil and elem[key].waypoint ~= nil then
      local speedAux = im.IntPtr(elem[key].waypoint.wpSpeed or 0)
      im.InputInt(" Arrival Speed##"..key, speedAux)
      elem[key].waypoint.wpSpeed = speedAux[0]
    end
  end
end
...
local function drawTransformsPreview()
  for i, lane in ipairs(dragData.strip.lanes) do
    if lane.spawn then
      -- player spawn
      debugDrawer:drawTextAdvanced(lane.spawn.transform.pos, String(string.format("spawn %d", i)), whiteColorF, true, false, blackColorI)
      debugDrawer:drawSphere(lane.spawn.transform.pos, 0.25, ColorF(1,1,1,0.2))
      local x, y, z, c = lane.spawn.transform.rot * vec3(1,0,0), lane.spawn.transform.rot * vec3(0,1,0), lane.spawn.transform.rot * vec3(0,0,1), lane.spawn.transform.pos
      debugDrawer:drawTriSolid(c-x/2, c+x/2, c+y, color(0,0,255,0.2*255))
    end

    if lane.endLine then
      --endLine
      debugDrawer:drawTextAdvanced(lane.endLine.transform.pos, String(string.format("endLine %d", i)), whiteColorF, true, false, blackColorI)
      debugDrawer:drawSphere(lane.endLine.transform.pos, 0.25, ColorF(1,1,1,0.2))
      local x, y, z = lane.endLine.transform.rot * vec3(lane.endLine.transform.scl.x,0,0), lane.endLine.transform.rot * vec3(0,lane.endLine.transform.scl.y,0), lane.endLine.transform.rot * vec3(0,0,lane.endLine.transform.scl.z)
      local c = lane.endLine.transform.pos
      -- topleft
    ...
    if lane.stage then
      -- stage
      debugDrawer:drawTextAdvanced(lane.stage.transform.pos, String(string.format("Stage %d", i)), whiteColorF, true, false, blackColorI)
      local x, y, z = lane.stage.transform.rot * vec3(lane.stage.transform.scl.x,0,0), lane.stage.transform.rot * vec3(0,lane.stage.transform.scl.y,0), lane.stage.transform.rot * vec3(0,0,lane.stage.transform.scl.z)
      local scl = (x+y+z)/2
      M.drawAxisBox(((-scl*2)+lane.stage.transform.pos),x*2,y*2,z*2,color(0,0,255,0.2*255))
      local pos = vec3(lane.stage.transform.pos)
...
local function drawPrefabHelper(label, prefabType)
  im.Text(label)
    im.SameLine()
    local boolptr = im.BoolPtr(dragData.strip.prefabs[prefabType].isUsed or false)
    ...
    dragData.strip.prefabs[prefabType].isUsed = boolptr[0]
    im.SameLine()
    if im.Button("Load..." .. "##"..label) then
      editor_fileDialog.openFile(function(data) dragData.strip.prefabs[prefabType].path = data.filepath end, {{"Prefab Files",".prefab.json"}}, false)
    end
    if dragData.strip.prefabs[prefabType].path then
      im.SameLine()
      im.TextColored(im.ImVec4(0.0, 1.0, 0.0, 1.0), "Loaded!")
      im.SameLine()
      if im.Button("x##"..label.."1") then
        dragData.strip.prefabs[prefabType].path = nil
      end
    elseif dragData.strip.prefabs[prefabType].isUsed then
      im.SameLine()
...
M.selectElement = function(index)
  ...
  transforms.spawn:set(elem.spawn.transform.pos, elem.spawn.transform.rot,  elem.spawn.transform.scl)
  ...
  transforms.stage:set(elem.stage.transform.pos, elem.stage.transform.rot, elem.stage.transform.scl)
  ...
  transforms.endLine:set(elem.endLine.transform.pos, elem.endLine.transform.rot, elem.endLine.transform.scl)
...
local function headsUpRace(ctxt)
  im.NewLine()
  ...
  for i, p in ipairs(dragData.phases) do
    ...
    local intAux = im.IntPtr(p.startedOffset)
    im.InputInt(" - Start Timer Offset##"..i.."tim", intAux)
    p.startedOffset = intAux[0]
  end
  im.Text("")
  ...
  im.SameLine()
  im.Checkbox("##".."prefabs", usingPrefabs)
  if usingPrefabs[0] then
    ...
    if dragData.strip.prefabs.christmasTree.isUsed then
      ...
      im.SameLine()
      local tType = search:beginSearchableSimpleCombo(im, "Select Type", dragData.strip.prefabs.christmasTree.treeType, treeTypes)
      if tType then
        dragData.strip.prefabs.christmasTree.treeType = tType
      end
      im.PopItemWidth()
    end
...
local function noCtxt(ctxt)
  im.NewLine()
  im.Text("There is no data for this context yet, this is still a WIP tool, thanks for reading this ^^")
end
...
local function loadDragData(filename)
  ...
  for _, lanes in ipairs(dragData.strip.lanes) do
  ...
  for _, value in pairs(dragData.strip.prefabs) do
...
local function dragMenu()
  ...
  im.SameLine()
  im.PushItemWidth(120)
  local ctxt = search:beginSearchableSimpleCombo(im, "context", "Select Context", allKnownContexts)
  if ctxt then
    dragData.context = ctxt
  end
  im.PopItemWidth()
  im.SameLine()
  im.TextColored(im.ImVec4(0.0, 1.0, 0.0, 1.0), "Selected Context: %s", dragData.context)
  if dragData.context ~= "" then
    funcContext[dragData.context](dragData.context)
  end
