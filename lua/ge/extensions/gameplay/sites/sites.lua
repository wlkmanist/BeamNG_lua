...
function C:finalizeSites()
  ...
  self.quadtreeLocations = kdtreePoint3d.new()
  self.quadtreeZones = kdtreeBox2d.new()
  self.quadtreeParkingSpots = kdtreePoint3d.new()
  ...
  for _, loc in ipairs(self.locations.sorted) do
    ...
    self.quadtreeLocations:preLoad(loc.id, loc.pos.x, loc.pos.y, loc.pos.z)
  end
  ...
  for _, ps in ipairs(self.parkingSpots.sorted) do
    ...
    self.quadtreeParkingSpots:preLoad(ps.id, ps.pos.x, ps.pos.y, ps.pos.z)
  end
...
function C:getClosestParkingSpot(pos)
  local id, dist = self.quadtreeParkingSpots:findNearest(pos.x, pos.y, pos.z)
  return self.parkingSpots.objects[id], dist
end
