...
function C:init(missionDir, vehicleTracker, notebook)
  ...
  self.intersectedPacenoteData_cs = nil
  self.intersectedPacenoteData_half = nil
  self.intersectedPacenoteData_ce = nil
...
function C:overrideTrackingPos(pos)
  self.trackingPos = pos
end

function C:detectCurrPoint()
  local pos = self.trackingPos or self.vehicleTracker:pos()
  -- find the point nearest the vehicle center, I presume.
  self.currPoint = self.driveline:findNearestPoint(pos, nil, false, false)
  -- that point probably wont be in front of the front wheels.
  self.trackingPos = nil
...
function C:onUpdate(nextPacenote)
  ...
  if not self.nextPacenote or (nextPacenote and nextPacenote.id ~= self.nextPacenote.id) or not nextPacenote then
  ...
  if self:intersectCorners() then
    ...
    -- local cachedPacenotes = self.currPoint.cachedPacenotes
    -- if cachedPacenotes then
    --   local msg = 'currPoint['..tostring(self.currPoint.id)..'] '
    --   if #tableKeys(cachedPacenotes) > 0 then
    --     for k,v in pairs(cachedPacenotes) do
    --       msg = msg .. k .. '('..v.pn.id .. ':' .. v.pn.name .. ') '
    --     end
    --   else
    --     msg = msg .. '{}'
    --   end
    ...
    self.intersectedPacenoteData_cs = self.currPoint.cachedPacenotes.cs
    self.intersectedPacenoteData_half = self.currPoint.cachedPacenotes.half
    self.intersectedPacenoteData_ce = self.currPoint.cachedPacenotes.ce
    --
...
function C:getIntersectedPacenoteDataCs()
  return self.intersectedPacenoteData_cs
end

function C:getIntersectedPacenoteDataCe()
  return self.intersectedPacenoteData_ce
end

function C:getIntersectedPacenoteDataHalf()
  return self.intersectedPacenoteData_half
end
...
function C:drawDebug()
  ...
  local wp_midWidth = wp_radius * 2

  if self.currPoint then
    drawnCache.points[self.currPoint.id] = true
    -- draw the current driveline point
    debugDrawer:drawSphere(
      self.currPoint.pos,
      1.0,
      ColorF(clr[1], clr[2], clr[3], alpha_shape*0.6)
    )
    ...
    for i,pnName in ipairs(sortedDistances) do
      ...
      -- draw distance, time, velocity text
  ...
  if self.nextPacenote then
    ...
    local side = wp_cs.normal:cross(vec3(0,0,1)) * (wp_radius - (wp_midWidth / 2))
    local pos = wp_cs.pos
    clr = cc.clr_green
    -- debugDrawer:drawSphere(
    --   pos,
    ...
    debugDrawer:drawSquarePrism(
      wp_cs.pos + side,
      wp_cs.pos + 0.25 * wp_cs.normal + side,
      Point2F(5, wp_midWidth),
      Point2F(0, 0),
      ColorF(clr[1], clr[2], clr[3], alpha_shape)
    )

    local wp_ce = self.nextPacenote:getCornerEndWaypoint()
    side = wp_ce.normal:cross(vec3(0,0,1)) * (wp_radius - (wp_midWidth / 2))
    local pos = wp_ce.pos
    clr = cc.clr_red
    -- debugDrawer:drawSphere(
    --   pos,
    ...
    debugDrawer:drawSquarePrism(
      wp_ce.pos + side,
      wp_ce.pos + 0.25 * wp_ce.normal + side,
      Point2F(5, wp_midWidth),
      Point2F(0, 0),
      ColorF(clr[1], clr[2], clr[3], alpha_shape)
    )
  ...
  for _,point in ipairs(self.driveline.points) do
    ...
    -- if true then
      ...
      debugDrawer:drawSphere(
        (pos),
        0.5,
        ColorF(clr[1], clr[2], clr[3], alpha_shape*0.2)
      )
